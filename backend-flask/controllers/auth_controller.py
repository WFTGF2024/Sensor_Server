"""
  ...
  def register():
      ...
      # 记录请求信息用于调试
      current_app.logger.info(f"Registration request data: username={data.get('username')}, password={'*' * len(data.get('password', '')) if data.get('password') else 'None'}, email={data.get('email')}, phone={data.get('phone')}")
      ...
      try:
          ...
          current_app.logger.error(f"Registration failed: {e.message}, error_code: {e.error_code}")
          raise e
